      ...
      var get = function (id, define, require) {
        ...
        var load = function (onsuccess, onfailure) {
          ...
          if (instance !== undefined) {
            ...
            onfailure('Modulator errors: could not resolve global [' + name + ']');
          }
        };
  ...
  function (ar, fn, map, piggybacker, stratifier) {
    var create = function (regulator, validator, onerror, define, require, demand) {
      ...
      var validate = function (onsuccess, results) {
        ...
        if (failed.length > 0)
          onerror('Fetcher errors: modules were not defined: ' + failed.join(', '));
        else
          onsuccess();
      };
...
kernel.module.loader = def(
  ...
  function (analyser) {
    var load = function (roots, deps, fetcher, oncontinue, onsuccess, onerror) {
      ...
      if (result.cycle)
        onerror('Dependency errors: a circular module dependency exists from ' + result.cycle.join(' ~> '));
  ...
  function (ar, obj, loader, fetcher) {
    var create = function (regulator, onerror) {
      ...
      var define = function (id, dependencies, definition) {
        if (id === undefined)
          onerror("Define errors: module id can not be undefined");
        else if (blueprints[id] !== undefined)
          onerror("Define errors: module '" + id + "' is already defined");
        else
      ...
      fs.exists(url, function (exists) {
        if (exists)
          fs.readFile(url, 'UTF-8', function (err, data) {
            if (err)
              error('Error reading file [' + url + '], errors [' + err + ']');
            else
              success(data);
          });
        else
  ...
  function () {
    ...
    var done = function (req, url, success, error) {
      if (req.status === 200 || req.status === 304)
        ...
        error('Transport errors: ' + req.status + ' ' + req.statusText + ' for resource: "' + url + '"');
    };
    ...
    var request = function (url, success, error) {
      ...
      if (req)
        ...
        error('Transport errors: browser does not support XMLHttpRequest.');
    };
...
module.error.error = def(
  ...
  function () {
    var die = function (msg) {
      throw msg || new Error('unknown errors');
    };
